...
public:
    ...
    {
        if (trees.count(projectName)) 
        {
            return trees[projectName].get();
        }
        return nullptr;
    }
    shared_ptr<ComplexNode> getComplexStructure(const string& projectName) 
    {
        if (complexStructures.count(projectName))
        {
            return complexStructures[projectName];
        }
        return nullptr;
    }
...
void demonstrateTree(ProjectManager& pm)
{
    ...
    if (treeFromPM) 
    {
        cout << "Project1 Tree Pre-Order: ";
        treeFromPM->traversePreOrder();
        cout << endl;
        cout << "Project1 Tree In-Order: ";
        treeFromPM->traverseInOrder();
        cout << endl;
        cout << "Project1 Tree Post-Order: ";
